...
float pidUpdate(PidObject* pid, const float measured, const bool isYawAngle)
{
  ...
  */
  ...
  outICandidate = pid->ki * integCandidate;
  outputUnsat = pid->outP + pid->outD + pid->outFF + outICandidate;

  if (pid->outputLimit != 0)
  {
    if ((outputUnsat > pid->outputLimit && pid->error > 0.0f) ||
        (outputUnsat < -pid->outputLimit && pid->error < 0.0f))
    {
      freezeIntegral = true;
    }
  }
  ...
  if (!freezeIntegral)
  {
    pid->integ = integCandidate;
  }

  pid->outI = pid->ki * pid->integ;
  output = pid->outP + pid->outD + pid->outI + pid->outFF;
