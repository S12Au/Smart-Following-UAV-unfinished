...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define PWM_MAX_SPEED 	2000
#define PWM_MIN_SPEED 	1000
#define NO_FIRST 			0
#define YES_FIRST 		1
/* USER CODE END PD */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	...
	uint8_t RxByte;
	uint16_t test=0;
	uint16_t pwmVal[4] = {0, 0, 0, 0};   //PWM占空比  
   uint8_t dir=1;
  ...
  //HAL_UARTEx_ReceiveToIdle_IT(&huart1, g_rxBuffer, g_rxLength);
  ...
  //HAL_TIM_IC_Start(&htim2, TIM_CHANNEL_2);//启动输入捕获
